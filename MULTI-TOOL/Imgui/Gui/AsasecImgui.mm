...
typedef struct {
    ...
    const char *category;          // Dinamik Kategori İsmi (Örn: "Combat", "Visuals", "Misc")
...
// Dinamik Kategori ve Switch Eklemek İçin
...
// Dinamik Kategori ve Buton Eklemek İçin
...
// Kayıtlı kategorileri benzersiz (unique) olarak toplamak için yardımcı fonksiyon
static int ASASECGetUniqueCategories(const char *categoriesOut[], int maxCategories) {
    int count = 0;
    for (int i = 0; i < gRegisteredFeatureCount; i++) {
        ...
        for (int j = 0; j < count; j++) {
            if (strcmp(categoriesOut[j], cat) == 0) {
                exists = true;
                break;
            }
        }
...
static float ASASECEase(float current, float target, float speed, float dt) {
...
static UIWindow *ASASECFindActiveWindow(void) {
    ...
    for (UIScene *scene in application.connectedScenes) {
        ...
        for (UIWindow *window in windowScene.windows) {
            if (!window || window.hidden || window.alpha <= 0.0) continue;
...
static void ASASECClampMenuToScreen(UIWindow *window) {
    ...
    float width = ASASECClampFloat(gMenuSize.x, kMenuMinWidth, targetMaxWidth);
    float height = gMenuCollapsed ? kHeaderHeight : ASASECClampFloat(gMenuSize.y, kMenuMinHeight, targetMaxHeight);

    gMenuSize.x = width;
    gMenuSize.y = height;

    float maxX = (float)size.width - width - margin;
    float maxY = (float)size.height - height - margin;

    if (maxX < margin) maxX = margin;
    if (maxY < margin) maxY = margin;

    gMenuPosition.x = ASASECClampFloat(gMenuPosition.x, margin, maxX);
    gMenuPosition.y = ASASECClampFloat(gMenuPosition.y, margin, maxY);
}
...
static ASASECSwitchAnimation *ASASECGetSwitchAnimationData(const char *label) {
    if (!label) return NULL;

    for (int i = 0; i < gSwitchAnimationCount; i++) {
        if (gSwitchAnimations[i].label && strcmp(gSwitchAnimations[i].label, label) == 0) {
            return &gSwitchAnimations[i];
        }
    }

    if (gSwitchAnimationCount >= 128) return NULL;
...
static bool ASASECModernSwitch(const char *label, bool *value) {
    ...
    bool clicked = ImGui::InvisibleButton("##switch", ImVec2(available, rowHeight));

    ImVec2 itemMin = ImGui::GetItemRectMin();
    ImVec2 itemMax = ImGui::GetItemRectMax();
    ImDrawList *draw = ImGui::GetWindowDrawList();

    ASASECSwitchAnimation *animationData = ASASECGetSwitchAnimationData(label);

    if (clicked) {
        *value = !(*value);
        if (animationData) animationData->pulse = 1.0f;
    }
    ...
    float progress = animationData ? animationData->progress : (*value ? 1.0f : 0.0f);
    float target = *value ? 1.0f : 0.0f;

    progress = ASASECEase(progress, target, 16.0f, dt);

    if (animationData) {
        animationData->progress = progress;
        animationData->pulse = ASASECEase(animationData->pulse, 0.0f, 12.0f, dt);
    }

    float pulse = animationData ? animationData->pulse : 0.0f;

    ImU32 cardBackground = hovered ? ASASECColor(0.075f, 0.100f, 0.145f, 0.98f) : ASASECColor(0.052f, 0.070f, 0.105f, 0.98f);
    ImU32 cardBorder = hovered ? ASASECColor(0.18f, 0.28f, 0.42f, 0.80f) : ASASECColor(0.12f, 0.17f, 0.25f, 0.75f);

    if (draw) {
        draw->AddRectFilled(itemMin, itemMax, cardBackground, 11.0f);
        draw->AddRect(ImVec2(itemMin.x + 0.5f, itemMin.y + 0.5f), ImVec2(itemMax.x - 0.5f, itemMax.y - 0.5f), cardBorder, 11.0f, 0, 1.0f);

        float switchX = itemMax.x - switchWidth - 14.0f;
        float switchY = itemMin.y + (rowHeight - switchHeight) * 0.5f;

        ImVec2 switchMin = ImVec2(switchX, switchY);
        ImVec2 switchMax = ImVec2(switchX + switchWidth, switchY + switchHeight);

        float bgR = 0.09f + (0.18f - 0.09f) * progress;
        float bgG = 0.13f + (0.52f - 0.13f) * progress;
        float bgB = 0.20f + (0.95f - 0.20f) * progress;
        ...
        draw->AddRectFilled(switchMin, switchMax, ASASECColor(bgR, bgG, bgB, 1.0f), switchHeight * 0.5f);
        draw->AddRect(switchMin, switchMax, ASASECColor(0.24f + 0.24f * progress, 0.32f + 0.32f * progress, 0.46f + 0.40f * progress, 0.85f), switchHeight * 0.5f, 0, 1.0f);

        float offX = switchMin.x + 13.0f;
        float onX = switchMax.x - 13.0f;
        float knobX = offX + (onX - offX) * progress;
        float knobY = switchMin.y + switchHeight * 0.5f;
        float knobRadius = 9.0f + pulse * 1.3f;

        if (pulse > 0.01f) {
            draw->AddCircle(ImVec2(knobX, knobY), 12.0f + pulse * 3.0f, ASASECColor(0.30f, 0.68f, 1.0f, pulse * 0.30f), 24, 1.6f);
        }

        draw->AddCircleFilled(ImVec2(knobX, knobY + 1.0f), knobRadius + 0.8f, ASASECColor(0.0f, 0.0f, 0.0f, 0.30f));
        draw->AddCircleFilled(ImVec2(knobX, knobY), knobRadius, ASASECColor(0.96f, 0.98f, 1.0f, 1.0f));
    }

    ImGui::SetCursorScreenPos(ImVec2(itemMin.x + 15.0f, itemMin.y + 8.0f));
    ImGui::TextColored(ImVec4(0.92f, 0.95f, 1.0f, 1.0f), "%s", label);

    ImGui::SetCursorScreenPos(ImVec2(itemMin.x + 15.0f, itemMin.y + 29.0f));
    ImGui::TextColored(ImVec4(0.38f, 0.44f, 0.53f, 1.0f), "%s", *value ? "Enabled" : "Disabled");
...
- (BOOL)pointInsideMenu:(CGPoint)point {
    ...
    return point.x >= gMenuPosition.x && point.x <= gMenuPosition.x + gMenuSize.x &&
           point.y >= gMenuPosition.y && point.y <= gMenuPosition.y + height;
}

- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    if (!gInitialized || !gMenuVisible) return nil;
    if ([self pointInsideMenu:point]) return self;
    return nil;
}

- (void)updateIOWithTouchEvent:(UIEvent *)event {
    if (!gInitialized || !event) return;
    ...
    for (UITouch *currentTouch in event.allTouches) {
        if (!currentTouch) continue;
        if (currentTouch.phase != UITouchPhaseEnded && currentTouch.phase != UITouchPhaseCancelled) {
            touching = YES;
            break;
        }
    }
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (!gMenuCollapsed && gMenuVisible) {
        ...
        if (point.x >= resizeIconX && point.y >= resizeIconY &&
            point.x <= gMenuPosition.x + gMenuSize.x && point.y <= gMenuPosition.y + gMenuSize.y) {
            gResizingMenu = YES;
        ...
        if (point.x >= gMenuPosition.x && point.x <= gMenuPosition.x + gMenuSize.x &&
            point.y >= gMenuPosition.y && point.y <= gMenuPosition.y + kHeaderHeight) {
            gDraggingMenu = YES;
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (gResizingMenu) {
        ...
        if (window) ASASECClampMenuToScreen(window);
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [self updateIOWithTouchEvent:event];
    ImGuiContext *ctx = ImGui::GetCurrentContext();
    if (ctx) ImGui::GetIO().MouseDown[0] = false;
...
static void ASASECApplyStyle(void) {
    ...
    style.ItemSpacing = ImVec2(9.0f, 12.0f);
...
static void ASASECSectionHeader(const char *title, const char *subtitle, ImU32 accent) {
    if (!title) return;
    ...
    if (subtitle) {
        ...
        ImGui::TextColored(ImVec4(0.34f, 0.40f, 0.49f, 1.0f), "%s", subtitle);
    }
    ImGui::PopStyleVar();
}
...
static void ASASECPageDescription(const char *text) {
    if (!text) return;
    ImGui::TextColored(ImVec4(0.42f, 0.47f, 0.56f, 1.0f), "%s", text);
}
...
- (void)drawInMTKView:(MTKView *)view {
    if (!gInitialized || !view || !gMetalDevice || !gCommandQueue || !gMetalBackendInitialized) return;
    ImGuiContext *ctx = ImGui::GetCurrentContext();
    if (!ctx) return;
    ...
    if (io.DeltaTime <= 0.0f || io.DeltaTime > 0.1f) io.DeltaTime = 1.0f / 60.0f;
    ...
    if (!gContentDragging && fabsf(gContentScrollVelocity) > 0.01f) {
        ...
        if (fabsf(gContentScrollVelocity) < 0.01f) gContentScrollVelocity = 0.0f;
    }

    if (gMenuVisible) {
        UIWindow *win = view.window;
        if (win) ASASECClampMenuToScreen(win);

        const float sidebarWidth = 145.0f;
        ImVec2 actualWindowSize = ImVec2(gMenuSize.x, gMenuCollapsed ? kHeaderHeight : gMenuSize.y);
        ...
        bool windowOpened = ImGui::Begin("##ASASEC_WINDOW", NULL, flags);
        ...
        if (windowOpened) {
            ...
            if (draw) {
                ...
                draw->AddRect(ImVec2(windowPos.x + 0.5f, windowPos.y + 0.5f), ImVec2(windowEnd.x - 0.5f, windowEnd.y - 0.5f), IM_COL32(48, 62, 84, 185), 22.0f, 0, 1.0f);
                draw->AddRectFilled(ImVec2(windowPos.x + 1.0f, windowPos.y + 1.0f), ImVec2(windowEnd.x - 1.0f, windowPos.y + kHeaderHeight), IM_COL32(10, 15, 25, 255), 21.0f, ImDrawFlags_RoundCornersTop);
                draw->AddLine(ImVec2(windowPos.x + 16.0f, windowPos.y + kHeaderHeight), ImVec2(windowEnd.x - 16.0f, windowPos.y + kHeaderHeight), IM_COL32(38, 48, 66, 220), 1.0f);
            }
            ...
            if (currentIO.Fonts && currentIO.Fonts->Fonts.Size > 0) ImGui::PushFont(currentIO.Fonts->Fonts[0]);
            ...
            if (currentIO.Fonts && currentIO.Fonts->Fonts.Size > 0) ImGui::PopFont();

            #pragma mark Control Panel Title
            if (!gMenuCollapsed) {
                float headerTitleX = windowSize.x - 230.0f;
                if (headerTitleX > 170.0f) {
                    ImGui::SetCursorPos(ImVec2(headerTitleX, 19.0f));
                    ImGui::TextColored(ImVec4(0.35f, 0.42f, 0.52f, 1.0f), "CONTROL PANEL");
                }
            }

            #pragma mark Collapse Button
            float collapseButtonX = windowSize.x - 96.0f;
            if (collapseButtonX < 0.0f) collapseButtonX = 0.0f;
            ...
            if (headerDraw) {
                ...
                ImU32 arrowColor = collapseHovered ? ASASECColor(0.42f, 0.74f, 1.0f, 1.0f) : ASASECColor(0.78f, 0.84f, 0.93f, 0.95f);
            ...
            if (closeButtonX < 0.0f) closeButtonX = 0.0f;
            ...
            if (closeDraw) {
                ...
                float size = 6.0f;
                ImU32 closeColor = closeHovered ? ASASECColor(1.0f, 0.45f, 0.52f, 1.0f) : ASASECColor(0.86f, 0.90f, 0.96f, 0.95f);
            ...
            if (!gMenuCollapsed && gMenuVisible) {
                if (draw) {
                    draw->AddRectFilled(ImVec2(windowPos.x, windowPos.y + kHeaderHeight), ImVec2(windowPos.x + sidebarWidth, windowEnd.y), IM_COL32(9, 14, 24, 255), 0.0f);
                    draw->AddRectFilled(ImVec2(windowPos.x, windowPos.y), ImVec2(windowPos.x + sidebarWidth, windowPos.y + kHeaderHeight), IM_COL32(9, 14, 24, 255), 16.0f, ImDrawFlags_RoundCornersTopLeft);
                    draw->AddRectFilled(ImVec2(windowPos.x, windowEnd.y - 22.0f), ImVec2(windowPos.x + sidebarWidth, windowEnd.y), IM_COL32(9, 14, 24, 255), 20.0f, ImDrawFlags_RoundCornersBottomLeft);
                    draw->AddLine(ImVec2(windowPos.x + sidebarWidth, windowPos.y + 62.0f), ImVec2(windowPos.x + sidebarWidth, windowEnd.y - 16.0f), IM_COL32(34, 43, 59, 210), 1.0f);
                }

                #pragma mark Sidebar (Dynamic Categories)
                ...
                int uniqueCategoryCount = ASASECGetUniqueCategories(uniqueCategories, 32);

                if (gSelectedPage >= uniqueCategoryCount) {
                    gSelectedPage = 0;
                }

                for (int i = 0; i < uniqueCategoryCount; i++) {
                    bool active = gSelectedPage == i;
                    float itemY = 72.0f + i * 54.0f;

                    if (active && draw) {
                        draw->AddRectFilled(ImVec2(windowPos.x + 9.0f, windowPos.y + itemY), ImVec2(windowPos.x + sidebarWidth - 9.0f, windowPos.y + itemY + 43.0f), IM_COL32(19, 48, 84, 255), 11.0f);
                        draw->AddCircleFilled(ImVec2(windowPos.x + 18.0f, windowPos.y + itemY + 21.5f), 3.0f, IM_COL32(80, 160, 255, 255));
                    }

                    char id[128];
                    snprintf(id, sizeof(id), "%s##page_%d", uniqueCategories[i], i);
                    ...
                    if (ImGui::Button(id, ImVec2(sidebarWidth - 20.0f, 43.0f))) {
                        if (gSelectedPage != i) {
                            gSelectedPage = i;
                            gPageAnimation = 0.0f;
                            gPageSlide = 18.0f;
                ...
                bool contentRootOpened = ImGui::BeginChild("##ContentRoot", ImVec2(contentWidth, contentHeight), false, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoScrollbar);

                if (contentRootOpened) {
                    ImGui::SetCursorPos(ImVec2(17.0f, 10.0f));

                    const char *currentCategoryName = (uniqueCategoryCount > 0 && gSelectedPage < uniqueCategoryCount) ? uniqueCategories[gSelectedPage] : "General";

                    ImGui::TextColored(ImVec4(0.94f, 0.97f, 1.0f, 1.0f), "%s", currentCategoryName);
                    ImGui::SameLine(0.0f, 7.0f);
                    ImGui::TextColored(ImVec4(0.30f, 0.36f, 0.45f, 1.0f), "/ ASASEC");

                    if (draw) {
                        draw->AddLine(ImVec2(windowPos.x + sidebarWidth + 16.0f, windowPos.y + 51.0f), ImVec2(windowEnd.x - 16.0f, windowPos.y + 51.0f), IM_COL32(32, 41, 56, 220), 1.0f);
                    }

                    ImGui::SetCursorPos(ImVec2(0.0f, 62.0f));
                    float scrollHeight = contentHeight - 62.0f;
                    if (scrollHeight < 100.0f) scrollHeight = 100.0f;

                    bool scrollableOpened = ImGui::BeginChild("##ScrollableContent", ImVec2(contentWidth, scrollHeight), false, ImGuiWindowFlags_NoScrollbar);

                    if (scrollableOpened) {
                        float fade = ASASECClampFloat(gPageAnimation, 0.0f, 1.0f);
                        ImGui::PushStyleVar(ImGuiStyleVar_Alpha, fade);
                        ImGui::SetCursorPosX(17.0f + gPageSlide);

                        ImU32 sectionAccent = ASASECColor(0.30f, 0.68f, 1.0f, 1.0f);
                        ASASECSectionHeader(currentCategoryName, "CONTROLS", sectionAccent);
                        ASASECPageDescription("Configure your loaded features");

                        ImGui::Dummy(ImVec2(0.0f, 10.0f));

                        float cardWidth = ImGui::GetContentRegionAvail().x - 22.0f;
                        if (cardWidth < 200.0f) cardWidth = 200.0f;

                        char childID[128];
                        snprintf(childID, sizeof(childID), "##Card_%s", currentCategoryName);

                        bool cardOpened = ImGui::BeginChild(childID, ImVec2(cardWidth, 294.0f), false, ImGuiWindowFlags_NoBackground);

                        if (cardOpened) {
                            for (int i = 0; i < gRegisteredFeatureCount; i++) {
                                if (gRegisteredFeatures[i].category && strcmp(gRegisteredFeatures[i].category, currentCategoryName) == 0) {
            ...
            if (!gMenuCollapsed && gMenuVisible && windowSize.x > 300.0f && windowSize.y > 220.0f) {
                ImDrawList *foreground = ImGui::GetForegroundDrawList();
                if (foreground) {
                    float iconRight = windowEnd.x - 9.0f;
                    float iconBottom = windowEnd.y - 9.0f;
                    ImVec2 iconCenter = ImVec2(iconRight - 11.0f, iconBottom - 11.0f);

                    ImU32 iconColor = gResizingMenu ? ASASECColor(0.42f, 0.72f, 1.0f, 1.0f) : ASASECColor(0.72f, 0.80f, 0.91f, 0.95f);
                    ImU32 iconGlow = ASASECColor(0.30f, 0.68f, 1.0f, gResizingMenu ? 0.18f : 0.07f);

                    foreground->AddCircleFilled(iconCenter, 13.0f, iconGlow, 24);
                    foreground->AddLine(ImVec2(iconCenter.x - 8.0f, iconCenter.y + 8.0f), ImVec2(iconCenter.x + 8.0f, iconCenter.y + 8.0f), iconColor, 2.2f);
                    foreground->AddLine(ImVec2(iconCenter.x + 8.0f, iconCenter.y - 8.0f), ImVec2(iconCenter.x + 8.0f, iconCenter.y + 8.0f), iconColor, 2.2f);
                    foreground->AddLine(ImVec2(iconCenter.x - 3.0f, iconCenter.y + 8.0f), ImVec2(iconCenter.x + 8.0f, iconCenter.y - 3.0f), iconColor, 2.0f);
                    foreground->AddLine(ImVec2(iconCenter.x - 8.0f, iconCenter.y + 3.0f), ImVec2(iconCenter.x + 8.0f, iconCenter.y - 8.0f), iconColor, 1.6f);
                }
            }
        }
...
void ASASECImGuiStart(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (gInitialized || gStarting) return;
        gStarting = YES;
