...
static BOOL gImGuiContextCreated = NO;
...
float rowWidth =
    available.x;
if (rowWidth < labelWidth + switchWidth + 28.0f)
    rowWidth =
        labelWidth + switchWidth + 28.0f;
ImVec2 rowSize =
    ImVec2(
        rowWidth,
        rowHeight
...
#pragma mark Touch IO

* (void)updateIOWithTouchEvent:(UIEvent *)event
    {
    if (!gInitialized ||
    !gImGuiContextCreated)
    return;
